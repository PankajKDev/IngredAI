    ...
    <div className=" bg-black w-full h-16 flex">
      ...
      <div className="w-2/3  ">
        ...
        <div className="justify-end relative group  items-center h-full mr-5 flex md:hidden">
          ...
          <MenuIcon onClick={()=>setIsOpen((state)=>!state)} className={` ${isOpen?"rotate-90":"rotate-0"} ease-in-out duration-700`}/>     
<SignedIn>
            <div className={`w-63 h-64  flex-col absolute bg-black right-0 top-0  mt-15 ${isOpen?"flex":"hidden"}`}>

          {isOpen?(
            <>
    {navLinks.map((item, index) => (
      <Link onClick={()=>setIsOpen((state)=>!state)} className="hoverLink p-4" href={item.linkname} key={index}>
              {item.name}
            </Link>
          ))}
          </>
